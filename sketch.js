...
// ============================================================
function preload() {
  bgArena = loadImage("assets/images/bgarena.jpg");
  floorTexture = loadImage("assets/images/floorTexture.png");
  ...
  splashMusic = loadSound("assets/sounds/splash.mp3");
}
...
function drawStartScreen() {
  if (!splashMusic.isPlaying()) splashMusic.loop();
  // Title
  fill(255);
  textAlign(CENTER);
  textSize(52);
  text("PORTAL WAR", width / 2, height / 2 - 60);
...
function drawWinScreen() {
  ...
  fill(winner === "P1" ? color(0,162,255) : color(255, 150, 30));
  textAlign(CENTER);
  textSize(56);
...
function drawArena() {
  ...
  push();
  tint(255, 50); // 60/255 opacity → subtle & dark
  image(bgArena, 0, 0, width, height);
  pop();

  // Draw tiled floor texture
  let tileW = 128; // adjust based on your texture size
  let tileH = 128;

  for (let x = 0; x < width; x += tileW) {
    for (let y = groundY; y < height; y += tileH) {
      image(floorTexture, x, y, tileW, tileH);
    }
  }
