# Week 3: Portal War

## Setup and Interaction Instructions

To run the sketch locally, open `index.html` in Google Chrome using Live Server.

Sound files must be present in `assets/sounds/` before running:

- `punch_1.wav` through `punch_9.wav`
- `win.wav`
- `background.mp3`
- `splash.mp3`

**Player 1 Controls:**

- Move: A / D
- Attack: F
- Block: G

**Player 2 Controls:**

- Move: Arrow Keys
- Attack: K
- Block: L

Press **ENTER** to start or rematch.

**Opening the Chrome Console**

- **Windows:** Press `F12` or `Ctrl + Shift + J`, then click the **Console** tab
- **Mac:** Press `Cmd + Option + J`

The console will show any errors in your sketch.

## Assets

| File                                        | Source                                            |
| ------------------------------------------- | ------------------------------------------------- |
| `assets/sounds/background.mp3` [1]             | DsTechnician, Robot Laser Carnage — Pixabay.com |
| `assets/sounds/splash.mp3` [2]                  | fidelfortune, Scifi intro-outro — Pixabay.com  |
| `assets/sounds/win.wav` [3]                    | listener4me, Win Sound Effect — OpenGameArt.org  |
| `assets/images/floorTexture` [4]               | Portal 1 textures - concrete, liquid, metal — OffiDocs.com |
| `assets/sounds/punch_3.wav` – `punch_9.wav` [5] | Punch SFX — OpenGameArt.org                     |
| `assets/sounds/punch_1.wav` [6] | Universfield, Metal Punch - Pixabay.com
| `assets/images/bgarena` [7]                    | qq3218162004, Portal 1 Wallpaper — Wallpapercave.com |
| `assets/sounds/punch_2.wav` [8] | Yodguard, warp magic(2) — Pixabay.com 

## References
[1] DSTechnician. n.d. Robot Laser Carnage. Pixabay.com. Retrieved May 27, 2026, from https://pixabay.com/music/video-games-robot-laser-carnage-113855/

[2] fidelfortune. n.d. Sci‑Fi Battle Music. Retrieved May 27, 2026, from https://pixabay.com/music/search/sci-fi%20battle/

[3] listener4me. n.d. _Win Sound Effect_. OpenGameArt.org. Retrieved May 1, 2026, from https://opengameart.org/content/win-sound-effect

[4] OffiDocs. n.d. Portal 1 Textures: Concrete, Liquid, Metal. Retrieved May 27, 2026, from https://www.offidocs.com/index.php/main-templates/photos-images-examples/400423-portal-1-textures-concrete-liquid-metal

[5] Punch SFX. n.d. OpenGameArt.org. Retrieved May 1, 2026, from https://opengameart.org/content/punch-sfx

[6] Universfield. n.d. Metal Punch. Pixabay.com. Retrieved May 27, 2026, from https://pixabay.com/sound-effects/film-special-effects-metal-punch-142334/

[7] qq3218162004. n.d. Portal 1 Wallpaper. Wallpapercave.com. Retrieved May 27, 2026, from https://wallpapercave.com/w/wp12562992

[8] Yodguard. n.d. warp magic(2). Pixabay.com. Retrieved May 27, 2026, from https://pixabay.com/sound-effects/film-special-effects-warp-magic-2-382390/
